...
camera.Open()
camera.ExposureTime.SetValue(600)
...
# --- Main Code ---
# --- Main Code ---
# === Define your folder and get all step files ===
target_folder = r"C:\Users\Yb\SLM\SLM\data\target_images\CNN"
step_files = sorted(glob.glob(os.path.join(target_folder, "tweezer_*.npy")))

# === Loop over each file ===
for step_path in step_files:
    basename = os.path.splitext(os.path.basename(step_path))[0]  # e.g., 'tweezer_step_0008'
    print(f"\n=== Running optimization for: {basename} ===")

    target_im_ideal = np.load(step_path)
    ...
    output_folder = r"C:\Users\Yb\SLM\SLM\data\target_images\WGS_CNN"
    # ******
    ...
    npy_save_path = os.path.join(output_folder, f"{basename}_phasehologram.npy")
    np.save(npy_save_path, phase)
    #print(f"Saved final phase to: {npy_save_path}")

    # # Apply FFT to get the tweezer pattern
    # u_fft = fft2(np.exp(1j * phase))  # Correctly apply FFT to the phase pattern
    # u_fft = fftshift(u_fft)
    ...
    # npy_save_path = os.path.join(output_folder, f"{basename}_wgs_fft_ampl_tweezer_completemap.npy")
    # np.save(npy_save_path, amplitude)
    # #print(f"Saved final phase to: {npy_save_path}")

    # phase = np.angle(u_fft)  # Phase information of tweezers
    # npy_save_path = os.path.join(output_folder, f"{basename}_wgs_fft_phase_tweezer_completemap.npy")
    # np.save(npy_save_path, phase)
    ...
    # coordinates_t = np.argwhere(target_im > 0)  # don't use peak_local_max for target
    ...
    # target_ampl_map = np.zeros_like(target_im)
    # # Assign ampl values at detected coordinates
    # for y, x in coordinates_ccd:
    #     target_ampl_map[y, x] = amplitude[y, x]
    # # plt.figure(figsize=(8, 6))
    # # plt.imshow(target_ampl_map)
    # # plt.colorbar(label="Amplitude")
    # # plt.title("Amplitude Spectrum of FFT (Tweezer Pattern)")
    # # plt.xlabel("Fourier X")
    # # plt.ylabel("Fourier Y")
    # # plt.show()
    # npy_save_path = os.path.join(output_folder, f"{basename}_wgs_fft_ampl_tweezer_singlepositions.npy")
    # np.save(npy_save_path, target_ampl_map)
    ...
    # # plt.figure(figsize=(10, 8))
    # # plt.imshow(amplitude, cmap='gray', alpha=0.5)
    ...
    # # Create an empty phase image of same size as the target
    # target_phase_map = np.zeros_like(target_im)
    # # Extract matched coordinates
    # ccd_coords = coordinates_ccd[row_ind]    # From FFT-detected peaks
    # tgt_coords = coordinates_t[col_ind]      # From ground-truth tweezer locations

    # # Pull the phase values from the FFT-phase image
    # phase_values = phase[ccd_coords[:, 0], ccd_coords[:, 1]]
    # # Fill the phase values into the target locations
    # for (y, x), phi in zip(tgt_coords, phase_values):
    #     target_phase_map[y, x] = phi
    # plt.figure(figsize=(8, 6))
    # plt.imshow(target_phase_map)
    # plt.colorbar(label="Amplitude")
    # plt.title("Amplitude Spectrum of FFT (Tweezer Pattern)")
    # plt.xlabel("Fourier X")
    # plt.ylabel("Fourier Y")
    ...
    # npy_save_path = os.path.join(output_folder, f"{basename}_wgs_fft_phase_tweezer_singlepositions.npy")
    # np.save(npy_save_path, target_phase_map)

    # # Convert target_im (amplitude) into complex field with added phase
    # target_complex = target_im * np.exp(1j * target_phase_map)

    # # Save the complex-valued image
    # npy_save_path = os.path.join(output_folder, f"{basename}_target_amplinput_and_phaseinput.npy")
    # np.save(npy_save_path, target_complex)

    npy_save_path = os.path.join(output_folder, f"{basename}_target_im.npy")
    np.save(npy_save_path, target_im)
    # npy_save_path = os.path.join(output_folder, f"{basename}_target_im_ideal.npy")
    # np.save(npy_save_path, target_im_ideal)
    #print(f"Saved complex amplitude+phase field to: {npy_save_path}")
    # plt.figure(figsize=(8, 6))
    # plt.imshow(np.abs(target_complex), cmap="inferno")
    # plt.colorbar(label="Amplitude")
    # plt.title("Amplitude of Complex Tweezer Field")
    # plt.xlabel("X")
    # plt.ylabel("Y")
    # plt.tight_layout()
    # #plt.show()
    # plt.figure(figsize=(8, 6))
    # plt.imshow(np.angle(target_complex))
    # plt.colorbar(label="Phase (radians)")
    # plt.title("Phase of Complex Tweezer Field")
    # plt.xlabel("X")
    # plt.ylabel("Y")
    # plt.tight_layout()
    # plt.show()
