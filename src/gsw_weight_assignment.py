...
target_folder = r"C:\Users\Yb\SLM\SLM\notebooks\Tweezer_step_images"
step_files = sorted(glob.glob(os.path.join(target_folder, "tweezer_step_*.npy")))
...
for step_path in step_files:
    ...
    camera.Open()
    camera.ExposureTime.SetValue(55)
    ...
    for rep in tqdm(range(n_rep), desc="Iterations", unit="it"):
        ...
        error_value = intensity_std(std_int)
        ...
        if rep % 30 == 0:
            plt.figure(figsize=(8, 6))
            plt.imshow(phase, cmap='gray', origin='upper')  # Flip origin if needed
            plt.title("Target Image After Fourier Scaling")
            plt.xlabel("X pixels")
            plt.ylabel("Y pixels")
            plt.colorbar(label="Intensity")
            plt.tight_layout()
            plt.figure(figsize=(8, 6))
            plt.imshow(std_int, cmap='gray', origin='upper')  # Flip origin if needed
            plt.title("Target Image After Fourier Scaling")
            plt.xlabel("X pixels")
            plt.ylabel("Y pixels")
            plt.colorbar(label="Intensity")
