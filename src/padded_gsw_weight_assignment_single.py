...
# Original and target shapes
original_shape = PS_shape.shape
target_shape = (4096, 4096)
padded_ampl = np.zeros(target_shape, dtype=np.float32)
# Compute starting indices to center the beam
start_y = (target_shape[0] - original_shape[0]) // 2
start_x = (target_shape[1] - original_shape[1]) // 2

# Paste the original beam into the center
padded_ampl[start_y:start_y + original_shape[0], start_x:start_x + original_shape[1]] = PS_shape

PS_shape = padded_ampl
...
for rep in tqdm(range(n_rep), desc="Iterations", unit="it"):
    ...
    phase_cropped = phase[start_y:start_y + 1200, start_x:start_x + 1920]
    #if rep==0:


    pattern_to_slm = np.flip(np.round((phase_cropped + np.pi) * 255 / (2 * np.pi)).astype(np.uint8), axis=1) + blazed_grating + phase_correction # this part is the phase pattern to send to the slm
    #else:
    ...
    std_int_padded = np.zeros((4096, 4096))

    crop_y = (std_int.shape[0] - 1200) // 2
    crop_x = (std_int.shape[1] - 1920) // 2
    std_int_cropped = std_int[crop_y:crop_y + 1200, crop_x:crop_x + 1920]
    std_int_padded[start_y:start_y + 1200, start_x:start_x + 1920] = std_int_cropped
    ...
    if rep>i:
        ...
        w=weights(w,target_im,w_prev,std_int_padded, coordinates, min_distance, num_peaks)
        ...
        #plt.pause(20)  # Small pause to update the plot

    error_value = intensity_std(std_int_padded, coordinates)
